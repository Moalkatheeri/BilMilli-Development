...
from app.core.exceptions import (
    StageNotFoundError, StageNotReadyError, InvalidTransitionError
)

logger = logging.getLogger("buildguard.services.stage")
...
class StageManager:
    ...
    async def create_stages_for_project(self, project: Project) -> List[ConstructionStage]:
        ...
        logger.info("Creating stages for project %s", project.id)
        """
        Auto-generate construction stages when a project is created.
    ...
    async def start_stage(self, stage_id: str, user_id: str) -> tuple[bool, str]:
        ...
        logger.info("Starting stage %s by user %s", stage_id, user_id)
        ...
        if not can_start:
            logger.warning("Cannot start stage %s: %s", stage_id, reasons)
            raise StageNotReadyError(stage_id, '; '.join(reasons))
        
        stage.status = StageStatus.IN_PROGRESS
        stage.actual_start = datetime.utcnow()
        
        await self.db.flush()
        logger.info("Stage %s (%s) started successfully", stage_id, stage.name)
    ...
    async def submit_for_approval(self, stage_id: str) -> tuple[bool, str]:
        ...
        if stage.status != StageStatus.IN_PROGRESS:
            raise InvalidTransitionError(stage.status.value, StageStatus.PENDING_APPROVAL.value)
        ...
        if incomplete_items:
            raise StageNotReadyError(stage_id, f"{len(incomplete_items)} checklist item(s) incomplete")
    ...
    async def approve_stage(self, stage_id: str, approved_by: str) -> tuple[bool, str]:
        ...
        logger.info("Approving stage %s by user %s", stage_id, approved_by)
        ...
        if stage.status != StageStatus.PENDING_APPROVAL:
            raise InvalidTransitionError(stage.status.value, StageStatus.APPROVED.value)
        ...
        logger.info("Stage %s (%s) approved", stage_id, stage.name)
