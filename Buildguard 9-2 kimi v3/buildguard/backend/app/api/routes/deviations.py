...
async def check_position_deviation(
    ...
    # Check access
    has_access = await check_project_access(request.project_id, current_user, db)
    if not has_access:
...
async def list_deviations(
    ...
    # Get all deviations for by_severity count
    all_deviations = await db.execute(query.order_by(DeviationEvent.detected_at.desc()))
    all_dev_list = all_deviations.scalars().all()
    
    # Count by severity
    by_severity = {}
    for d in all_dev_list:
        by_severity[d.severity.value] = by_severity.get(d.severity.value, 0) + 1
    
    # Get paginated results
    result = await db.execute(
        query.order_by(DeviationEvent.detected_at.desc())
        .offset(skip)
        .limit(limit)
    )
...
async def get_deviation(
    ...
    # Check access
    has_access = await check_project_access(deviation.project_id, current_user, db)
    if not has_access:
        raise HTTPException(status_code=403, detail="Access denied")
...
async def update_deviation(
    ...
    """Update deviation status and notes."""
    ...
    if "status" in request:
        try:
            deviation.status = DeviationStatus(request["status"])
        except ValueError:
            raise HTTPException(status_code=400, detail=f"Invalid status: {request['status']}")
    
    if "reviewer" in request:
        deviation.reviewed_by = request["reviewer"]
        deviation.reviewed_at = datetime.utcnow()
    
    if "notes" in request:
        deviation.notes = request["notes"]
    
    if "rectification_notes" in request:
        deviation.rectification_notes = request["rectification_notes"]
        if request.get("status") == "rectified":
            deviation.rectified_at = datetime.utcnow()
    
    if "actual_cost" in request:
        deviation.actual_cost = request["actual_cost"]
