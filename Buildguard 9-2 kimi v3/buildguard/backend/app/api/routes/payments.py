...
from app.schemas.payment import (
    ...
    BlockingIssuesResponse, PaginatedPaymentGateListResponse, PaginatedDlpTicketListResponse
)
...
@router.get("/gates/project/{project_id}", response_model=PaginatedPaymentGateListResponse)
async def list_payment_gates(
    ...
    return PaginatedPaymentGateListResponse(
        items=list(payments),
        total=total
    )
...
async def get_payment_gate(
    ...
    # Check access
    has_access = await check_project_access(payment.project_id, current_user, db)
    if not has_access:
        raise HTTPException(status_code=403, detail="Access denied")
...
@router.get("/tickets/project/{project_id}", response_model=PaginatedDlpTicketListResponse)
async def list_dlp_tickets(
    ...
    return PaginatedDlpTicketListResponse(
        items=list(tickets),
        total=total
    )
