...
// Backend API URL - Uses config for dynamic environment detection
...
class ApiService {
  ...
  // ==================== AI / PRO-BACKEND PROXY ====================
  async analyzePhoto(fileUri, metadata = {}) {
    const formData = new FormData();
    ...
    const response = await this.client.post('/ai/photos/analyze', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
      timeout: 60000,
  ...
  async analyzeFloorplan(fileUri, projectId = null) {
    const formData = new FormData();
    ...
    formData.append('file', {
      uri: fileUri,
      name: fileInfo.name || 'floorplan.dwg',
      type: fileInfo.type || 'application/octet-stream',
    });
    ...
    const response = await this.client.post('/ai/floorplan/analyze', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
      timeout: 120000,
  ...
  async convertFloorplanTo3D(fileUri, projectId = null) {
    const formData = new FormData();
    ...
    const response = await this.client.post('/ai/floorplan/convert-to-3d', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
      timeout: 300000, // 5 minutes for 3D conversion
    });
  ...
  async getModelStatus(modelId) {
    const response = await this.client.get(`/ai/models/${modelId}/status`);
    return response.data;
  }

  async generateModelFromText(description) {
    const response = await this.client.post('/ai/models/generate-from-text', {
      description
    });
    return response.data;
  }
