...
class PaymentService:
    ...
    async def request_payment_release(
        ...
        if not can_release:
            ...
            return False, f"Payment blocked: {'; '.join(reasons)}"
    ...
    async def approve_payment(
        ...
        if payment.status != PaymentStatus.PENDING:
            return False, f"Payment must be pending approval (current: {payment.status.value})"
        ...
        if not can_release:
            ...
            return False, f"Payment no longer releasable: {'; '.join(reasons)}"
