...
from app.schemas.stage import (
    ...
    ChecklistUpdateResponse, CanStartResponse, StageListResponse
)

router = APIRouter()


@router.get("/project/{project_id}", response_model=StageListResponse)
...
async def get_stage(
    ...
    # Check access
    has_access = await check_project_access(stage.project_id, current_user, db)
    if not has_access:
        raise HTTPException(status_code=403, detail="Access denied")
...
async def can_start_stage(
    ...
    # Get stage to check project access
    result = await db.execute(
        select(ConstructionStage).where(ConstructionStage.id == stage_id)
    )
    stage = result.scalar_one_or_none()
...
async def submit_for_approval(
    stage_id: str,
    request: Optional[StageSubmitRequest] = None,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(get_current_user)
):
