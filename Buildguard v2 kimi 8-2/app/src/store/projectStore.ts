...
export const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  login: async (email: string, password: string) => {
    ...
    try {
      ...
      toast.success('Welcome back!', { description: `Logged in as ${user.full_name}` });
    } catch (err: any) {
  ...
  register: async (data) => {
    ...
    try {
      ...
      setToken(token);
      set({ isAuthenticated: true, token, user, loading: false });
      toast.success('Account created!', { description: 'Welcome to BuildGuard Pro' });
    } catch (err: any) {
  ...
  startStage: async (stageId) => {
    ...
    try {
      ...
      toast.success('Stage started', { description: 'Construction can begin' });
    } catch (err: any) {
      const message = err.message || 'Failed to start stage';
      set({ error: message, loading: false });
      toast.error('Failed to start stage', { description: message });
    }
  },
  ...
  submitStage: async (stageId) => {
    ...
    try {
      ...
      const stagesRes = await stagesApi.list(project.id);
      set({ stages: stagesRes.items, loading: false });
      toast.success('Stage submitted', { description: 'Awaiting approval' });
    } catch (err: any) {
      const message = err.message || 'Failed to submit stage';
      set({ error: message, loading: false });
      toast.error('Failed to submit stage', { description: message });
    }
  },
  ...
  approveStage: async (stageId) => {
    ...
    try {
      ...
      toast.success('Stage approved', { description: 'Payment gate unlocked' });
    } catch (err: any) {
      const message = err.message || 'Failed to approve stage';
      set({ error: message, loading: false });
      toast.error('Failed to approve stage', { description: message });
    }
  },
  ...
  requestPaymentRelease: async (paymentId) => {
    ...
    try {
      ...
      toast.success('Payment release requested', { description: 'Awaiting approval' });
    } catch (err: any) {
      const message = err.message || 'Failed to request payment release';
      set({ error: message, loading: false });
      toast.error('Payment blocked', { description: message });
    }
  },
  ...
  approvePayment: async (paymentId) => {
    ...
    set({ loading: true, error: null });
    try {
      const result = await paymentsApi.approve(paymentId, user.id);
      const paymentsRes = await paymentsApi.listGates(project.id);
      set({ payments: paymentsRes.items, loading: false });
      toast.success('Payment approved', { description: `${result.amount.toLocaleString()} AED released` });
    } catch (err: any) {
      const message = err.message || 'Failed to approve payment';
      set({ error: message, loading: false });
      toast.error('Failed to approve payment', { description: message });
    }
  },
